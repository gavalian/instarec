 ...
 * @author tyson
 */
public class ElasticScatteringValidator_instaskim {
    
    public ElasticScatteringValidator_instaskim(){
    }
    ...
    public void process(String file, int limEvs,String endName, double threshold, int targPID, double beamE, Boolean requireMatchedEl, Boolean print, int desired_sector, int... tags){
      ...
      while(r.hasNext() && count<limEvs){
        ...
        Leaf htcc = ev.readLeaf(1,12,32200,1);
        ev.read(recpart);
        ev.read(reccal);
        ev.read(triggerbank);
        ...
        for(int row=0;row<recpart.getRows();row++){
          for (int row2 = 0; row2 < recpart.getRows(); row2++) {
            ...
            int hasTrig=1;//hasTriggerEl(triggerbank);
            ...
            double totHTCC=1;//getHTCCSectorSum(htcc,sector); //&& totHTCC!=0
            ...
            if( Math.abs(p[4])>=2000 && Math.abs(p[4])<4000 && p[5]==1 && p[0]==targPID){ //Math.abs(p[4])>4000
    ...
    public static void main(String[] args){
      ...
      String endName="_rga_wCuts_th0p05_phiCut5_pFD_instaskim"; //_phiCut5 eg 175-185, 10 otherwise

      double resp_threshold=0.05;
      double beamE=10.6;
        
      ElasticScatteringValidator_instaskim dp = new ElasticScatteringValidator_instaskim();
