 ...
 * @author tyson
 */
public class TwoPionTagger extends DataWorker {

  Schema recparticleSchema = null;
  double beamE=10.6;
  double targetMass=0.938272;

  public TwoPionTagger(double be,double m){
  ...
  public void calcExc(double[] pip, double[] pim, double[] el, double[] exc,double beamE, double tm){
      
    double elE=Math.sqrt(square(el[1])+square(0.000511));
    double pipE=Math.sqrt(square(pip[1])+square(0.13957));
    double pimE=Math.sqrt(square(pim[1])+square(0.13957));
    double pM=tm;
    ...
    double E_m = beamE + tm - (elE+pipE+pimE);
  ...
  public void execute(DataEvent event) {
      ...
      for(int row=0;row<bpart.getRows();row++){
        for (int row2 = 0; row2 < bpart.getRows(); row2++) {
          for (int row3 = 0; row3 < bpart.getRows(); row3++) {
            ...
            fillRECPart(bpart,row2,pim);
            fillRECPart(bpart,row3,pip);
            fillRECPart(bpart,row,el);


            if(Math.abs(pim[4])>=2000 && Math.abs(pim[4])<4000 && pim[0]==-211 ){
              if(Math.abs(pip[4])>=2000 && Math.abs(pip[4])<4000 && pip[0]==211){
                if(Math.abs(el[4])>=2000 && Math.abs(el[4])<4000 && el[5]==-1 && row!=row2){
                  calcExc(pip,pim, el, exc,beamE,targetMass);

                  //System.out.printf("beamE %f tm %f MM %f\n",beamE,targetMass,exc[1]);

                  //cut on missing mass, large for plots and fits
                  if(exc[1]>0.2 && exc[1]<2.1){
                    ((Event) event).setEventTag(2211);
                  }
